...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
    ...
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, 0, _player);
    BattlegroundQueueTypeId bgQueueTypeIdRandom = BattlegroundMgr::BGQueueTypeId(BATTLEGROUND_RB, 0, _player);
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    BattlegroundTypeId bgTypeId = BattlegroundTypeId(bgTypeId_);
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, type, _player);
    BattlegroundQueue& bgQueue = sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);
...
void WorldSession::HandleBattlefieldLeaveOpcode(WorldPacket& recvData)
{
    ...
    // Not Allow Leave Arena Before 3 Min
    if (Battleground* bg = _player->GetBattleground())
        if (bg->isArena() && bg->isRated())
            if (bg->GetStartTime() <= 180000)
                if (bg->GetStatus() != STATUS_WAIT_LEAVE)
                    return;
    // Not Allow Leave Arena Before 3 Min
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
    ...
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, arenatype, _player);
    PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), _player->getLevel());
    if (!bracketEntry)
        return;
