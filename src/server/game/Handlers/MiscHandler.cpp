...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        // Script For Account Vips -- Who Vip Tag
        std::string pname;
        if (target->GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true)
        {
            pname = "|cffff0000<VIP>|r";
    ...
    // Script For Fake Player In Who
    uint32 CountFake = 0;
    uint32 FakeTedad = sWorld->getIntConfig(CONFIG_FOR_FAKE_PLAYER_TEDAD);
    uint32 FakeTaqsim = sWorld->getIntConfig(CONFIG_FOR_FAKE_PLAYER_TAQSIM);
    float FakeZarib = sWorld->getFloatConfig(CONFIG_FOR_FAKE_PLAYER_ZARIB);

    if (matchcount >= FakeTedad)
        CountFake = floor((matchcount / FakeTaqsim) * FakeZarib + matchcount);
    else
        CountFake = matchcount;

    data.put(0, displaycount);                           // insert right count, count displayed
    data.put(4, CountFake);                              // insert right count, count of matches
    // Script For Fake Player In Who
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    // Script For Account Vips -- Instant Logout
    bool instantLogout = (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) && !GetPlayer()->IsInCombat()) || GetPlayer()->IsInFlight() || GetPlayer()->GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true || HasPermission(rbac::RBAC_PERM_INSTANT_LOGOUT);
    // Script For Account Vips -- Instant Logout
