    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player* player, Unit* u)
        {
            ...
            if(sConfigMgr->GetBoolDefault("PrepatchGI.Added", false))
                ...
                // Script For Account Vips -- Exp
                float Rate_Kill_Exp = (player->GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getRate(VIP_RATE_KILL_EXP) : sWorld->getRate(RATE_XP_KILL);

                gain = uint32(gain * Rate_Kill_Exp);
                // Script For Account Vips -- Exp
                }
            }
