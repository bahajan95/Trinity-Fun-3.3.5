...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    //                                            79           80           81             82                83           84        85
                                             " questItem6, movementId, RegenHealth, mechanic_immune_mask, flags_extra, ScriptName, name2 "
    ...
    do
    {
		...
		creatureTemplate.Name2				= fields[85].GetString();
...
void ObjectMgr::LoadChatFilter()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 ChatFilter words. DB table `chat_filter` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u chat filter words in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
//script For Anti Fosh
...
//Script For Custom Requirements
void ObjectMgr::LoadCustomRequirements()
{
	uint32 oldMSTime = getMSTime();

	_customRequirementsStore.clear();   // for reload case (test only)

	QueryResult result = WorldDatabase.Query("SELECT `id`, `RequiredLevel`, `MegaPoints`, `VipLevel`, `HonorPoints`, `ArenaPoints`, "
		"`VotePoints`, `OnlinePoints`, `RaceMaskAllow`, `ClassMaskAllow`, `DuelRating`, `Arena2v2Rating`, `Arena3v3Rating`, `Arena5v5Rating`, "
		"`NeedItem1`, `NeedItem2`, `NeedItem3`, `NeedItem4`, `NeedItem5`, `NeedItem6`, `NeedItem7`, `NeedItem8`, `NeedItem9`, "
	    "`ItemCount1`, `ItemCount2`, `ItemCount3`, `ItemCount4`, `ItemCount5`, `ItemCount6`, `ItemCount7`, `ItemCount8`, `ItemCount9`, `rewardid` "
		"FROM `custom_requirements`");

	if (!result)
	{
		TC_LOG_ERROR("server.loading", ">> Loaded 0 custom requirement. DB table `custom_requirements` is empty!");
		return;
	}
	...
	do
	{
		...
		uint32 i = 0;
		uint32 id = fields[i++].GetUInt32(); // 0

		CustomRequirements cr;

		cr.requiredlevel = fields[i++].GetUInt8(); // 1
		cr.megapoints = fields[i++].GetUInt32(); // 2
		cr.viplevel = fields[i++].GetUInt8(); // 3
		cr.honorpoints = fields[i++].GetUInt32(); // 4
		cr.arenapoints = fields[i++].GetUInt32(); // 5
		cr.votepoints = fields[i++].GetUInt32(); // 6
		cr.onlinepoints = fields[i++].GetUInt32(); // 7
		cr.raceMask = fields[i++].GetUInt32(); // 8
		cr.classMask = fields[i++].GetUInt32(); // 9
		cr.duelrating = fields[i++].GetUInt16(); // 10
		cr.arena2v2rating = fields[i++].GetUInt16(); // 11
		cr.arena3v3rating = fields[i++].GetUInt16(); // 12
		cr.arena5v5rating = fields[i++].GetUInt16(); // 13

		for (uint32 j = 0; j < MAX_CUSTOM_REQUIREMENTS_NEEDITEM; ++j)
		{
			int32 itemid = fields[i++].GetInt32();
			cr.needitem[j] = uint32(itemid);
			ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemid);
			if (!proto && itemid > 0)
			{
				TC_LOG_ERROR("server.loading", ">> Requirement id %u in `custom_requirements` has invalid item entry %u", id, uint32(itemid));
				cr.needitem[j] = 0;
			}
		}

		for (uint32 j = 0; j < MAX_CUSTOM_REQUIREMENTS_COUNTITEM; ++j)
		{
			int32 itemcount = fields[i++].GetInt32();
			cr.itemcount[j] = uint32(itemcount);
		}

		cr.rewardid = fields[i++].GetUInt32();

		_customRequirementsStore[id] = cr;

		++count;
	} while (result->NextRow());

	TC_LOG_INFO("server.loading", ">> Loaded %u custom requirements in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
//Script For Custom Requirements

//Script For Custom Reward
void ObjectMgr::LoadCustomReward()
{
	uint32 oldMSTime = getMSTime();

	_customRewardStore.clear();   // for reload case (test only)

	QueryResult result = WorldDatabase.Query("SELECT `id`, `Level`, `MegaPoints`, `HonorPoints`, `ArenaPoints`, `VotePoints`, `OnlinePoints`,"
		"`NeedItem1`, `NeedItem2`, `NeedItem3`, `NeedItem4`, `NeedItem5`, `NeedItem6`, `NeedItem7`, `NeedItem8`, `NeedItem9`, "
		"`ItemCount1`, `ItemCount2`, `ItemCount3`, `ItemCount4`, `ItemCount5`, `ItemCount6`, `ItemCount7`, `ItemCount8`, `ItemCount9` "
		"FROM `custom_reward`");

	if (!result)
	{
		TC_LOG_ERROR("server.loading", ">> Loaded 0 custom reward. DB table `custom_reward` is empty!");
		return;
	}
	...
	do
	{
		...
		cr.level = fields[i++].GetUInt8(); // 1
		cr.megapoints = fields[i++].GetUInt32(); // 2
		cr.honorpoints = fields[i++].GetUInt32(); // 3
		cr.arenapoints = fields[i++].GetUInt32(); // 4
		cr.votepoints = fields[i++].GetUInt32(); // 5
		cr.onlinepoints = fields[i++].GetUInt32(); // 6

		for (uint32 j = 0; j < MAX_CUSTOM_REWARD_NEEDITEM; ++j)
		{
			int32 itemid = fields[i++].GetInt32();
			cr.needitem[j] = uint32(itemid);
			ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemid);
			if (!proto && itemid > 0)
			{
				TC_LOG_ERROR("server.loading", ">> Reward id %u in `custom_reward` has invalid item entry %u", id, uint32(itemid));
				cr.needitem[j] = 0;
			}
		}

		for (uint32 j = 0; j < MAX_CUSTOM_REWARD_COUNTITEM; ++j)
		{
			int32 itemcount = fields[i++].GetInt32();
			cr.itemcount[j] = uint32(itemcount);
		}

		_customRewardStore[id] = cr;

		++count;
	} while (result->NextRow());

	TC_LOG_INFO("server.loading", ">> Loaded %u custom reward in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
//Script For Custom Reward
