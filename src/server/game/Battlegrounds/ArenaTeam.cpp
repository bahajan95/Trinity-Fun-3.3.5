...
void ArenaTeam::MemberWon(Player* player, uint32 againstMatchmakerRating, int32 MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        if (itr->Guid == player->GetGUID())
        {
            ...
            int32 Item_Id = sWorld->getIntConfig(CONFIG_FOR_ARENA_ITEM_ID);
            int32 Count_Item = sWorld->getIntConfig(CONFIG_FOR_ARENA_COUNT_ID);

            if (sWorld->getBoolConfig(CONFIG_FOR_ARENA_ENABLE))
                player->AddItem(Item_Id, Count_Item);
            // Script For Arena Reward Item
