...
BattlegroundQueueTypeId BattlegroundMgr::BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType, Player* player)
{
	time_t t = time(NULL);
	...
	case BATTLEGROUND_AB:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '3'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
	...
	case BATTLEGROUND_AV:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '1'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
	...
	case BATTLEGROUND_EY:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '7'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
	...
	case BATTLEGROUND_IC:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '30'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
	...
	case BATTLEGROUND_RB:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '32'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
	...
	case BATTLEGROUND_SA:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '9'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
	...
	case BATTLEGROUND_WS:
	{
							QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '2'");
							Field* Fields = Result->Fetch();
							uint32 closed = Fields[0].GetUInt32();
							if (closed == 1)
								return BATTLEGROUND_QUEUE_NONE;
        ...
        case BATTLEGROUND_RV:
			...
			case ARENA_TYPE_2v2:
				...
				if (sWorld->getBoolConfig(CONFIG_ARENA_2v2_TEAM_ENABLE) == true)
				{
					QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '6'");
					Field* Fields = Result->Fetch();
					uint32 closed = Fields[0].GetUInt32();
					if (closed == 1)
					{
						ChatHandler(player->GetSession()).PSendSysMessage("|cffFF0000Arena:|r Arena Haye 2v2 Dar Hale Hazer Close Mibashand.");
						return BATTLEGROUND_QUEUE_NONE;
					}
					else
			...
			case ARENA_TYPE_3v3:
				// Script For Disabling Arena
				if (sWorld->getBoolConfig(CONFIG_ARENA_3v3_TEAM_ENABLE) == true)
				{
					QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '6'");
					Field* Fields = Result->Fetch();
					uint32 closed = Fields[0].GetUInt32();
					if (closed == 1)
					{
						ChatHandler(player->GetSession()).PSendSysMessage("|cffFF0000Arena:|r Arena Haye 3v3 Dar Hale Hazer Close Mibashand.");
						return BATTLEGROUND_QUEUE_NONE;
					}
					else
			...
			case ARENA_TYPE_5v5:
				// Script For Disabling Arena
				if (sWorld->getBoolConfig(CONFIG_ARENA_5v5_TEAM_ENABLE) == true)
				{
					QueryResult Result = WorldDatabase.Query("SELECT closed FROM battleground_template WHERE id = '6'");
					Field* Fields = Result->Fetch();
					uint32 closed = Fields[0].GetUInt32();
					if (closed == 1)
					{
						ChatHandler(player->GetSession()).PSendSysMessage("|cffFF0000Arena:|r Arena Haye 5v5 Dar Hale Hazer Close Mibashand.");
						return BATTLEGROUND_QUEUE_NONE;
					}
					else
