...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    if (!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        if (Player* plr2 = ObjectAccessor::FindPlayer(group->Players.begin()->first))
        {
            ...
            BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(group->BgTypeId, group->ArenaType, plr2);
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
		...
		BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, bg->GetArenaType(), nullptr);
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bg->GetTypeID(), bg->GetArenaType(), player);
    uint32 queueSlot = player->GetBattlegroundQueueIndex(bgQueueTypeId);
