...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
        ...
        if (incremental)
        {
            ...
            // Script For Account Vips -- Rep
            float Rate_Rep = (_player->GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getRate(VIP_RATE_REP) : sWorld->getRate(RATE_REPUTATION_GAIN);

            standing = int32(floor((float)standing * Rate_Rep + 0.5f));
            // Script For Account Vips -- Rep
