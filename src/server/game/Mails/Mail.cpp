...
{
    PreparedStatement* Query = CharacterDatabase.GetPreparedStatement(CHAR_GET_EXTERNAL_MAIL);
    PreparedQueryResult Result = CharacterDatabase.Query(Query);
    ...
    do
    {
        ...
        Query = CharacterDatabase.GetPreparedStatement(CHAR_DEL_EXTERNAL_MAIL);
        Query->setUInt32(0, id);
        Trans->Append(Query);
    }
