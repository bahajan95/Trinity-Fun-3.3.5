...
#include "../../../scripts/Custom/System/system_transmogrification.h"
#include "../../../scripts/Custom/System/system_megapoint.h"
#include "../../../scripts/Custom/System/system_votepoint.h"
#include "../../../scripts/Custom/System/system_warlords.h"
...
#pragma warning(default:4355)
	...
	m_op_time = sWorld->getIntConfig(CONFIG_ONLINE_POINT_TIME) * IN_MILLISECONDS;
    ...
    // Script For Receive Message
    GLOBAL_MESSAGE_SEE = true;
    KILL_PLAYER_MESSAGE = true;
    VIP_PLAYER_ACTIVE = false;
    GM_CHECK_CHAT = false;
    // Script For Receive Message

    // Script For Global Channel
    GLOBAL_TIME_MESSAGE = true;
    // Script For Global Channel
}
...
bool Player::Create(uint32 guidlow, CharacterCreateInfo* createInfo)
{
    ...
    if (!info)
    // Fix Spam Warning From Console
    // {
    //    TC_LOG_ERROR("entities.player", "Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid race/class pair (%u/%u) - refusing to do so.", GetSession()->GetAccountId(), m_name.c_str(), createInfo->Race, createInfo->Class);
        return false;
    // }
    // Fix Spam Warning From Console
...
//Script for Add OPS
bool Player::AddOps(uint32 ops)
{
	uint32 account_id = GetSession()->GetAccountId();
	QueryResult result = LoginDatabase.PQuery("SELECT `id`, `ops` FROM `account_onlinepoint` WHERE `id` = '%u'", account_id);

	if (!result)
	{
		LoginDatabase.PExecute("INSERT INTO `account_onlinepoint` (`id`, `ops`) VALUES (%u, %u);", account_id, ops);
		return true;
	}
	else
	{
		LoginDatabase.PExecute("UPDATE `account_onlinepoint` SET `ops` = `ops` + %u WHERE `id` = %u", ops, account_id);
		return true;
	}
}

uint32 Player::GetOps()
{
	uint32 account_id = GetSession()->GetAccountId();
	QueryResult result = LoginDatabase.PQuery("SELECT `ops` FROM `account_onlinepoint` WHERE `id` = '%u'", account_id);
	if (result)
	{
		Field *field = result->Fetch();
		int32 ops = field->GetInt32();
		return ops;
	}
	else
		return 0;
...
void Player::Update(uint32 p_time)
{
	...
	if (m_op_time > 0)
    {
		if (p_time >= m_op_time)
        {
			uint32 ops = sWorld->getIntConfig(CONFIG_ONLINE_POINT_VALUE);
			uint32 times = sWorld->getIntConfig(CONFIG_ONLINE_POINT_TIME);
			if (!isAFK())
			{
				AddOps(ops);
				ChatHandler(GetSession()).PSendSysMessage("|CFF009C1AReward:|r |CFFFF0000%u |r Online Points Be Shoma Dade Shod.", ops);
			}
			else
				ChatHandler(GetSession()).PSendSysMessage("|CFF009C1AReward:|r Be Dalil AFK Boodan Be Shoma Online Points Dade Nashod.", ops);
			m_op_time = +times * IN_MILLISECONDS;
        }
        else
			m_op_time -= p_time;
    }
    ...
    // Fix Warrior Enrage Talent
    if ((HasAura(12880) && HasAura(57522)) || (HasAura(14201) && HasAura(57522)) || (HasAura(14202) && HasAura(57522)) || (HasAura(14203) && HasAura(57522)) || (HasAura(14204) && HasAura(57522)))
...
void Player::UpdateDefense()
{
    ...
    uint8 SkillDefense = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getIntConfig(VIP_RATE_DEFENSE) : sWorld->getIntConfig(CONFIG_SKILL_GAIN_DEFENSE);
    // Script For Account Vips

    if (UpdateSkill(SKILL_DEFENSE, SkillDefense))
...
bool Player::UpdateCraftSkill(uint32 spellid)
{
    ...
    for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
    {
        if (_spell_idx->second->skillId)
        {
            ...
            // Script For Account Vips
            uint32 craft_skill_gain = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getIntConfig(VIP_RATE_CRAFTING) :  sWorld->getIntConfig(CONFIG_SKILL_GAIN_CRAFTING);
            // Script For Account Vips
...
bool Player::UpdateGatherSkill(uint32 SkillId, uint32 SkillValue, uint32 RedLevel, uint32 Multiplicator)
{
    ...
    // Script For Account Vips
    uint32 gathering_skill_gain = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getIntConfig(VIP_RATE_GATHERING) : sWorld->getIntConfig(CONFIG_SKILL_GAIN_GATHERING);
    // Script For Account Vips
...
void Player::UpdateWeaponSkill(WeaponAttackType attType)
{
    ...
    // Script For Account Vips
    uint32 weapon_skill_gain = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getIntConfig(VIP_RATE_WEAPON) : sWorld->getIntConfig(CONFIG_SKILL_GAIN_WEAPON);
    // Script For Account Vips
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            // Script For Account Vips
            float Rate_Explorer = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ?  sWorld->getRate(VIP_RATE_EXPLORER) : sWorld->getRate(RATE_XP_EXPLORE);
            // Script For Account Vips
            ...
            else
            {
                ...
                if (diff < -5)
                {
                    XP = uint32(sObjectMgr->GetBaseXP(getLevel()+5) *  Rate_Explorer); // Script For Account Vips
                }
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr->GetBaseXP(areaEntry->area_level)*exploration_percent/100 * Rate_Explorer); // Script For Account Vips
                }
                else
                {
                    XP = uint32(sObjectMgr->GetBaseXP(areaEntry->area_level) * Rate_Explorer); // Script For Account Vips
                }
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    // Script For Account Vip -- Honor Vip
    float Rate_Honor = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getRate(VIP_RATE_HONOR) : sWorld->getRate(RATE_HONOR);
    ...
    if (sWorld->getBoolConfig(CONFIG_PVP_TOKEN_ENABLE) && pvptoken)
    {
        ...
        if (victim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            // Script For Account Vip -- Rate Drop Token
            uint32 itemId = sWorld->getIntConfig(CONFIG_PVP_TOKEN_ID);
            int32 count = sWorld->getIntConfig(CONFIG_PVP_TOKEN_COUNT);
            int32 countvip = sWorld->getIntConfig(VIP_TOKEN_DROP_COUNT);

            if (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true)
                AddItem(itemId, countvip);
            else
...
void Player::UpdateArea(uint32 newArea)
{
    ...
    if (area && area->IsSanctuary() || GetAreaId() == 2252)    // in sanctuary
    {
...
//                break;
            ...
            case ITEM_MOD_SPELL_POWER:
				if (getClass() == CLASS_PALADIN)
					...
					ApplySpellPowerBonus(int32(val * 1), apply);
                break;
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
			...
			if (CostQuery)
			{
				...
				CustomRequirementsContainer::const_iterator cr = require->find(require_id);
				if (cr != require->end())
				{
					if (cr->second.viplevel > GetSession()->GetVipLevel())
					{
						std::string icon = Warlords->GetItemIcon(pProto->ItemId, 20, 20, -14, 0);
						std::stringstream vip;
						vip << "|TInterface/ICONS/Achievement_Quests_Completed_08:15:15:-14:0|t Your vip level must be |cff006400 " << cr->second.viplevel << "|r";
						PlayerTalkClass->ClearMenus();
						PlayerTalkClass->SendGossipMenu(vip.str() , GetGUID());
...
void Player::MoveItemFromInventory(uint8 bag, uint8 slot, bool update)
{
    ...
    {
		Warlords->SendPacketItem(this, it->GetEntry(), 0, NULL);
...
void Player::SwapItem(uint16 src, uint16 dst)
{
	...
	Warlords->SendPacketItem(this, pSrcItem->GetEntry());
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    // Script For Account Vips
    float Rate_Quest = (GetReceiveMessage(CHECK_VIP_PLAYER_ACTIVE) == true) ? sWorld->getRate(VIP_RATE_QUEST) : sWorld->getRate(RATE_XP_QUEST);

    uint32 XP = rewarded ? 0 : uint32(quest->XPValue(this) * Rate_Quest); // Script For Account Vips
    // Script For Account Vips
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    else if (mapEntry && mapEntry->IsBattlegroundOrArena())
    {
        ...
        {
            BattlegroundQueueTypeId bgQueueTypeId = sBattlegroundMgr->BGQueueTypeId(currentBg->GetTypeID(), currentBg->GetArenaType(), this);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
				Warlords->SendPacketItem(this, item->GetEntry());
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
	...
	if (CostQuery)
	{
		...
		if (!Warlords->CheckRequirement(this, require_id))
			return false;
	}
	//Script For Cost Item
	...
	//Script For Cost Item
	//Script for Multivendor
	Warlords->SendPacketItem(this, pProto->ItemId);	
...
void Player::RemoveItemDependentAurasAndCasts(Item* pItem)
{
    for (AuraMap::iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end();)
    {
        ...
        // Fix Warrior Change Weapon While Bladestorm And Shield Wall With Bladestorm
        if (HasAura(46924))
        {
            ToPlayer()->RemoveAura(871);
            ++itr;
            continue;
        }
        // Fix Warrior Change Weapon While Bladestorm And Shield Wall With Bladestorm
...
// Script Get Receive Message
bool Player::GetReceiveMessage(int Message)
{
    switch (Message)
    {
        case CHECK_GLOBAL_MESSAGE_SEE:  return GLOBAL_MESSAGE_SEE;  break;
        case CHECK_VIP_PLAYER_ACTIVE:   return VIP_PLAYER_ACTIVE;   break;
        case CHECK_KILL_PLAYER_MESSAGE: return KILL_PLAYER_MESSAGE; break;
        case CHECK_GM_CHECK_CHAT:       return GM_CHECK_CHAT;       break;
