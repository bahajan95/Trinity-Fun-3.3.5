...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
	...
	if (tm.tm_hour == sWorld->getIntConfig(CONFIG_ARENA_START_TIME))
	{
		WorldDatabase.PExecute("UPDATE battleground_template SET closed = 1");
	}

	if (tm.tm_hour == sWorld->getIntConfig(CONFIG_ARENA_END_TIME))
	{
		WorldDatabase.PExecute("UPDATE battleground_template SET closed = 0");
	}
