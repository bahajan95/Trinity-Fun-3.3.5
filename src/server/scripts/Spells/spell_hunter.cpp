...
class spell_hun_masters_call : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_masters_call_SpellScript : public SpellScript
        {
            ...
            // Fix Master Call Hunter Pet
            SpellCastResult CheckPetDeathCast()
            {
                if (Player* player = GetCaster()->ToPlayer())
                    for (Unit::ControlList::const_iterator itr = player->m_Controlled.begin(); itr != player->m_Controlled.end(); ++itr)
                        if (Creature* HunterPet = (*itr)->ToCreature())
                            if (HunterPet->IsAlive() && HunterPet->GetOwnerGUID() == player->GetGUID() && HunterPet->GetCreatureType() == CREATURE_TYPE_BEAST && HunterPet->IsWithinDist(player, 100.0f, false))
                                if (!HunterPet->IsWithinLOSInMap(player))
            ...
            {
                // Fix Master Call Hunter Pet
                OnCheckCast += SpellCheckCastFn(spell_hun_masters_call_SpellScript::CheckPetDeathCast);
                // Fix Master Call Hunter Pet
