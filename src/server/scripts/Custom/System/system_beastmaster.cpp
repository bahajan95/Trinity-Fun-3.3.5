...
{
 QueryResult result;
 result = WorldDatabase.PQuery("SELECT `entry`, `name` FROM `custom_beastmaster` WHERE `cat_number` = 0 ORDER BY `entry` ASC");
 ...
 {
 //you are too poor
 ChatHandler(player->GetSession()).PSendSysMessage("You don't have enough money.", LANG_UNIVERSAL, player);
 player->CLOSE_GOSSIP_MENU();
 }
 else
 {
...
{
 QueryResult result;
 result = WorldDatabase.PQuery("SELECT `entry`, `name` FROM `custom_beastmaster` WHERE `cat_number` = 1 ORDER BY `entry` ASC");
...
{
 QueryResult result;
 result = WorldDatabase.PQuery("SELECT `entry`, `name` FROM `custom_beastmaster` WHERE `cat_number` = 2 ORDER BY `entry` ASC");
