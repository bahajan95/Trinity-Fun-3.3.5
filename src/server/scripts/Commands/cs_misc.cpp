...
#include "../custom/system/system_megapoint.h"
...
public:
    ...
    {
		static ChatCommand GoldenCommandTable[] =
        {
            { "add",      rbac::RBAC_PERM_COMMAND_TRINITY_ADD,        false, &HandleAdd1Command, "", NULL },
            { NULL,       0,                                       false, NULL,                       "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "additem",          rbac::RBAC_PERM_COMMAND_ADDITEM,          false, &HandleAddItemCommand,          "", NULL },
			{ "trinity",          rbac::RBAC_PERM_COMMAND_TRINITY,          false, NULL,             "", GoldenCommandTable },
    ...
    static bool HandleAdd1Command(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        Player* meTarget = handler->getSelectedPlayer();
        if (!meTarget)
            meTarget = me;
        ...
        if (count < 0)
        {
            meTarget->DestroyItemCount(itemId, -count, true, false);
            handler->PSendSysMessage(LANG_REMOVEITEM, itemId, -count, handler->GetNameLink(meTarget).c_str());
            return true;
        }
        ...
        InventoryResult msg = meTarget->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, count, &noSpaceForCount);
        ...
        Item* item = meTarget->StoreNewItem(dest, itemId, true, Item::GenerateItemRandomPropertyId(itemId));

        // remove binding (let GM give it to another player later)
        if (me == meTarget)
            for (ItemPosCountVec::const_iterator itr = dest.begin(); itr != dest.end(); ++itr)
                if (Item* item1 = me->GetItemByPos(itr->pos))
                    item1->SetBinding(false);

        if (count > 0 && item)
        {
            me->SendNewItem(item, count, false, true);
            if (me != meTarget)
                meTarget->SendNewItem(item, count, true, false);
        }
