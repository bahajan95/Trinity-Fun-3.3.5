...
public:
	...
	ChatCommand* GetCommands() const
	{
		...
		static ChatCommand reloadCommandTable[] =
		{
			...
			{ "custom_requirements", rbac::RBAC_PERM_COMMAND_RELOAD_CUSTOM_REQUIREMENTS, true, &HandleReloadCustomRequirementsCommand, "", NULL },
			{ "custom_reward", rbac::RBAC_PERM_COMMAND_RELOAD_CUSTOM_REWARD, true, &HandleReloadCustomRewardCommand, "", NULL },
			...
			{ "chat_filter",                  rbac::RBAC_PERM_COMMAND_RELOAD_CHAT_FILTER, true,  &HandleReloadLoadChatFilterCommand,             "", NULL },
			{ NULL, 0, false, NULL, "", NULL }
		};
	...
	static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, const char* args)
	{
		...
		handler->SendGlobalGMSysMessage("|cddff0000[GoldenGame]:|r Table `item_template` reloaded|r");
		return true;
	}
	...
	static bool HandleReloadCustomRequirementsCommand(ChatHandler* handler, const char* /*args*/)
	{
		TC_LOG_INFO("misc", "Loading Custom Requirements... (`custom_requirements`)");
		sObjectMgr->LoadCustomRequirements();
		handler->SendGlobalGMSysMessage("DB table `custom_requirements` reloaded.");
		return true;
	}

	static bool HandleReloadCustomRewardCommand(ChatHandler* handler, const char* /*args*/)
	{
		TC_LOG_INFO("misc", "Loading Custom Reward... (`custom_reward`)");
		sObjectMgr->LoadCustomReward();
		handler->SendGlobalGMSysMessage("DB table `custom_reward` reloaded.");
		return true;
	}
